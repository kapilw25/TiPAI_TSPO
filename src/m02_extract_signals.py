...
def get_images_to_process(conn: sqlite3.Connection) -> list:
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT i.image_id, i.generation_prompt, i.image_path, i.base_id, i.variation
        FROM images i
...
def extract_all_signals(image: Image.Image, prompt: str, image_id: str, config: Config) -> ImageSignals:
    """Extract all 3 signals for a single image."""
    # Parse prompt to get expected objects
    expected_objects = parse_prompt(prompt)
    expected_nouns = [obj.full_phrase for obj in expected_objects]

    # Signal 1: SAM + CLIP object detection
    detected = extract_signal_1(image, prompt, expected_objects, config)

    # Signal 2: Grad-CAM
    gradcam_path = extract_signal_2(image, prompt, image_id, config)

    # Signal 3: Object gaps
    gaps = extract_signal_3(expected_objects, detected, config)
...
def process_all_images(config: Config) -> None:
    ...
    for image_id, prompt, image_path, base_id, variation in tqdm(images, desc="Extracting"):
        ...
        try:
            ...
            signals = extract_all_signals(image, prompt, image_id, config)
